...
const IncidentDetailsModal = ({ isOpen, onClose, incidentId, onUpdate }) => {
  ...
  const handleSuccessfulOperation = async () => {
    try {
      setUpdating(true);
      setError('');
      await supabaseService.updateIncidentStatus(incidentId, 'resolved', 'Successful operation - incident resolved.');
    ...
    >
      {loading ? (
        ...
        <div>
          ...
          {/* Request Assistance Section - Barangay only; status auto-set to Assigned when sent */}
